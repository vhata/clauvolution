...
pub struct GeographyStats {
    ...
    /// aquatic band, taken on the tile the organism starts its move on (the
    /// pre-move tile, the one whose movement cost it pays). Food eaten on
    /// water below uses the post-move tile instead.
    ...
    /// Food items regenerated by `food_regeneration_system` on deep water,
    /// shallow water and land. Only regeneration: the initial stock and
    /// nutrient-rain bursts are not counted, so this is not the same base
    /// as `food_eaten`, which counts items from every source.
    pub food_spawned: [u64; 3],
    /// Food items eaten, from any source, by any organism that eats one,
    /// standing on deep and shallow water. The eater's tile is taken after
    /// its move (the post-move tile), unlike `water_ticks`.
...
impl GeographyStats {
    pub fn add(&mut self, other: &GeographyStats) {
        ...
        for d in 0..WATER_DEPTH_KEYS.len() {
            self.food_eaten_on_water[d] += other.food_eaten_on_water[d];
        }
        for t in 0..3 {
    ...
    pub fn minus(&self, other: &GeographyStats) -> GeographyStats {
        ...
        for w in 0..WATER_DEPTH_KEYS.len() {
            d.food_eaten_on_water[w] -= other.food_eaten_on_water[w];
        }
        for t in 0..3 {
