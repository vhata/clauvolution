...
- [SIM] `system-ordering-convention` — **Decide one convention for simulation system ordering.** Some systems use explicit `.chain()` and others rely on default Bevy ordering within a tuple, so the intent behind any given ordering is unclear.
  ...
  - Source: docs/ROADMAP.md (Known tech debt), 2026-09-16
- [BRAIN] `speciation-stalls-after-founding` — **Find out why new species so rarely form after the founding pass.** In `--headless 5000` runs on 2026-09-23, seed 42 formed no species at all after its 60 founders (25 still alive at tick 5000), and seed 3 formed 13 after its 21 founders, 9 of them in one pass at tick 2701. A watcher mostly sees the founders thin out rather than lineages branching.
  - Starting point: A member leaves its species only when it is more than 1.3 from that species' representative and more than 1.0 from every other one, and the representative is re-drawn each pass as whichever living member the query yields first, so a species that drifts carries its representative with it. Measure how far members sit from their representative over a run before touching the threshold, hysteresis or representative choice; the sweeps in the "Species classification" DECISIONS entry counted species at the end of a run, not speciation events.
  - Source: todo/species-id-reuse-after-extinction branch, 2026-09-23
