...
- **Shelf width.** At weight 0.8, ramp 48 and warp 48, no shelf joins two regions on any of the eight audit seeds up to a width of 12. The first bridge appears at 14, on seeds 3 and 99; at 16 five seeds are bridged and at 20 six. **8 was taken**, well under the first bridge, so the narrowest strait on the eight seeds still has deep water across it for at least a few tiles beyond two shelf widths. That margin is specific to these seeds. The independent review of #92 ran the check over 204 seeds (0..199 plus 1234, 2024, 31337 and 65535) and found a shelf joining two major regions at width 8 on 18 of them, about 9%, some between large continents (seed 159: 39,864 and 39,726 tiles); on six of those the largest bridge-free width is 5 to 7. Filed as `shelf-bridges-on-some-seeds` in `TODO.md`. The cost of the choice is that shelves are narrow: 12% to 15% of the water is shallow, against 25% to 78% before.
...
**Species from tick 9000**, as the mean over the late window rather than the single sample at 15000, step 3 -> step 4: seed 1 32 -> 39, 2 50 -> 46, 3 52 -> 40, 7 41 -> 46, 42 44 -> 57, 99 48 -> 25, 314 44 -> 42, 1000 45 -> 37. Most of this is trajectory noise: swings of the same size happened between the earlier phase 2 audits on unchanged or barely changed maps (seed 314 from 47 in the baseline to 25 after the movement change to 44 in step 3; seed 42 from 81 to 44). Seed 99 is the exception. Its late window runs 21 to 36 with a mean of 25, wholly below step 3's minimum of 40, and its mean is below the minimum of every earlier phase 2 audit (31 in the baseline and after the movement change, 40 in step 3), which suggests an effect of its map. One run per seed cannot confirm it; the threshold sweep in step 7 re-reads species counts on the final traits.
...
- A second run per seed. Each row above is one trajectory; seed 99's species count in particular needs a second run or step 7's sweep before being read as the map's effect.
