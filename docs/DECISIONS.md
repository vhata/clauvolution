...
**Accepted tradeoff:** one more component and one more thing for a future system between `predation_system` and `death_system` to remember to filter on. Kills and predation deaths are now equal by construction (3389 and 3389 at the seed above). Removing the corpse income moves the ecology a long way; see the tuning entry that follows.
