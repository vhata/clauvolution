...
- **Diet trait** — `diet` genome trait in -1..1 (herbivore to carnivore) with plant and animal digestion efficiencies of `((1 ∓ diet) / 2)²`, so a generalist digests a quarter of each. Plant efficiency scales what a grazer keeps from a bite and what an eater keeps from a food item; animal efficiency scales the killer's pyramid share. Shown in Inspect with both efficiencies, as an average in the Graphs trait plot and stats grid, in the headless summary, and as `avg_diet` in `--dump-history`.
