...
- **Controls drift.** Compare the controls table in `README.md` and the controls summary in `CLAUDE.md` against the `KeyCode` matches in `keyboard_to_events_system` (`crates/clauvolution_sim/src/lib.rs`), `clauvolution_render`, and `clauvolution_ui`. A key bound in code and absent from both documents, or documented and no longer bound, is a `Docs` finding.
