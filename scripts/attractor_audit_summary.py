...
by cause, kills, grazer kills of consumers, grazes by output, plant kills by
consumers with the energy they kept, average body size, and average diet. Also reports whether the two
...
def main(d):
    ...
    for p in paths:
        ...
        try:
            ...
            share = "-"
        print(f"| {seed} | {run} | {r['plants']} | {r['grazers']} | {r['hunters']} | {r['omnivores']} | {share} | {r['species']} | {r['deaths']} | {r['starv']} | {r['pred']} | {r['old']} | {r['dis']} | {r['kills']} | {r['gkills']} | {r['grazes']} | {r['pkills']} | {r['body']} | {r['diet']} | {r['light']} | {r['ready']} |")
