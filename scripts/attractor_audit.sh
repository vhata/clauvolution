...
echo "commit $(git rev-parse --short HEAD), ticks $TICKS, seeds ${SEEDS[*]}, $REPEATS runs each" | tee "$OUT_DIR/README.txt"

run_one() {
  local seed=$1 run=$2 label="seed$1-run$2"
  echo "$(date +%T) start $label"
  "$BIN" --headless "$TICKS" --seed "$seed" --dump-history "$OUT_DIR/$label.csv" \
